 ...
 *
 * ## No structural sniffing
 *
 * {@link toErrorEnvelope} takes an {@link OperationalError} — the closed union of
 * these 21 classes — and reads `_tag`, `error`, and `suggestions` off it
 * directly. It never inspects an unknown value's shape to guess whether it is an
 * error, and it never falls back to a default code: an unrepresented failure is
 * a compile error at the call site, not a mystery `ERR_UNKNOWN` on the wire.
 */
