...
## 1. The skill — `skills/ears-validated/SKILL.md`

Copy `SKILL.md` into `skills/ears-validated/` of your plugin or project. This is the primary surface that tells Claude when and how to invoke the MCP. Trigger conditions, mental model, workflows, and anti-patterns are all here. Lazy-loaded — only consumes context once it's matched.
...
The skill is designed to be the destination of the `CL-RIGOR → contract-unclear → EARS` route. When that classifier fires, Claude loads `ears-validated`, runs the **author_new_spec** workflow against the session's spec directory, and at the end exports the SysML JSON that Plan reads. ERPAVal's existing markdown `spec.md` becomes a derived view; the graph is authoritative.
...
3. Expected: Claude invokes the `ears-validated` skill, calls `requirements_list` first, then `requirement_create` for each criterion, then `relationship_add` for the derives DAG, then `analysis_run`, and shows you the findings.
