...
## The three governing invariants (the doctrine)

Every rule below is an application of three invariants. They are the business logic — the tool's entire value is that it holds them even when holding them means saying less.

### 1. Sound modulo atomization — a false negative is honest, a false positive is the cardinal sin

symspec may **miss** a conflict (report clean over a real contradiction) but must **never fabricate** one (report a contradiction that does not exist). A miss is the honest failure direction; a fabricated conflict in a verdict-eligible tier is the one thing the doctrine forbids. This is why `atomize` normalizes conservatively (no stemming/lemmatization — `issues` ≠ `issue`), why `extractNumericPredicates` returns `[]` rather than guess, and why a conflict is provable only when two responses resolve to the **same atom** at opposite polarity — `src/formal/atomize.ts:120`, `src/formal/numeric.ts:21`, `src/formal/atomize.ts:1`. The live proof of this rule: the numeric quantity key deliberately does NOT strip trailing comparator words, because doing so once fabricated a false `FND_NUMERIC_CONTRADICTION` (see "Why the quantity key stays conservative" below) — `src/formal/numeric.ts:211`.

### 2. Propose / decide — fuzzy signals only PROPOSE; only a committed artifact the SMT layer consults can change a verdict

Embeddings, quantity-alias guesses, and relational-shape detection can only PROPOSE candidates (info-tier findings suggesting a `glossary add` / `antonym add`). Nothing fuzzy ever asserts a contradiction. The only thing that changes a verdict is a committed, agent-reviewed artifact (the glossary or antonym table) that the deterministic `atomize`/`quantityKey` path consults — `src/formal/semantic.ts:1`, `src/formal/atomize.ts:144`, `src/formal/numeric.ts:160`. The corollary the adversarial critic hardened: lenient normalization that helps a PROPOSE signal must never touch the DECIDE key — the sound path gets the STRICTER normalization, the propose path the looser one (`.erpaval/solutions/architecture/normalization-for-a-propose-signal-must-not-touch-the-decide-key.md`).

### 3. Demotion-only — propose-only findings and coverage stats may push `verified` toward FALSE, never toward TRUE

`data.verified` is `verified = demotions.length === 0` — literally the empty-demotion-list check — `src/pipeline/check.ts:1289`. Propose-only findings and coverage gaps append to the one demotion block, so they can only DEMOTE `verified` toward abstention; only the deterministic decide tier (z3 unsat, LIA/LRA unsat) leaves the demotion list empty. A fuzzy signal can raise the alarm; only the deterministic decide tier can sound the all-clear — `src/pipeline/check.ts:286`. `verified: false` is not a dead end but a work list: each demotion carries the exact discharging command, and applying them converges to `verified: true` — `src/pipeline/check.ts:281`. The propose-only codes are gathered in `PROPOSE_ONLY_FND_CODES` so they are structurally barred from promoting — `src/pipeline/check.ts:413`.
...
### Why the quantity key stays conservative (the soundness rule, `labelBefore`)

The numeric quantity key is derived from the noun phrase immediately before the comparator, and it deliberately does **NOT** strip trailing comparator words (`within`/`under`/`over`/`above`/`below`/`exceeding`) — the explicit soundness comment at `src/formal/numeric.ts:211`. There is no syntactic way to tell a leaked compound-bound word ("complete the infusion **within** at most 30 min") from a phrasal-verb noun tail ("the carry **over** at least 100 mb", "roll over", "turn over") — both are `[word][comparator][number]`. An early cut that stripped these unconditionally collapsed "carry over" and "carry" onto one key and fabricated a false `FND_NUMERIC_CONTRADICTION` — a false positive in the decide tier, the cardinal sin under invariant 1. The adversarial critic caught it where the green suite (only the target case) did not. The rule: the DECIDE key stays conservative (a conservative key can only cause a MISS); the lenient object-suffix matching lives only in the propose-only quantity-alias detector below — `src/formal/numeric.ts:218`.

### Quantity-alias candidate (`FND_QUANTITY_ALIAS_CANDIDATE`, info, propose-only) — the tractable blind spot, SOLVED via propose→decide

The numeric tier proves a conflict only when two bounds land on the SAME per-system quantity key. The same physical quantity described with two different verbs splits into two keys and is never compared: "complete the infusion within ≤30 min" (key `complete_the_infusion`) vs "run the infusion for ≥60 min" (key `run_the_infusion`) — jointly UNSAT, but the keys never meet, so pre-hardening `check` reported `verified=true` (GitHub issue #2, reproducer a) — `src/formal/quantity-alias.ts:4`. `findQuantityAliasCandidates` fires ONLY on the high-precision signature of "one quantity, two verbs": same system, same non-empty trigger, quantity labels sharing a common object SUFFIX but differing in leading verb, directionally OPPOSED comparators, comparable unit — `src/formal/quantity-alias.ts:160`, `:174`, `:188`. It emits an info-tier finding carrying the exact `symspec glossary add "<a>" "<b>"` command — `src/formal/quantity-alias.ts:207`. This is a textbook propose→decide application: the finding only PROPOSES (and DEMOTES `verified`); committing the suggested alias routes both labels to one quantity key so the existing LIA solver proves the real `FND_NUMERIC_CONTRADICTION` (DECIDE). It must NOT auto-merge: "transmit the packet" and "receive the packet" also share an object but are genuinely different quantities — auto-merging would fabricate a conflict — `src/formal/quantity-alias.ts:18`. The `opposed()` guard fires only on shapes that could be jointly unsatisfiable once unified; two same-direction bounds only tighten, so they never demote — `src/formal/quantity-alias.ts:114`.

### Relational / aggregate blind spot (`FND_RELATIONAL_UNCHECKED`, info, propose-only) — the intractable blind spot, DETECTED-AND-DEMOTED, never solved

Three conflict families are NOT soundly recoverable from prose by any deterministic extractor: (1) aggregate/conservation (N reservations summing past capacity), (2) cross-quantity arithmetic (`end = start + duration`), (3) emergent structural impossibility (odd-cycle 2-coloring, pigeonhole) — reproducer b is five sensors in a ring each required to differ from its neighbor with only two channels, graph-theoretically impossible, yet all pairs individually satisfiable so the pairwise tier reports `verified=true` — `src/formal/relational.ts:4`. Inferring the finite resource or arithmetic relation would require guessing, and guessing violates invariant 1. So `findRelationalUnchecked` DETECTS the structural shape and DEMOTES with an honest "reasoning not attempted" caveat — it never asserts a conflict, so it cannot fabricate one — `src/formal/relational.ts:100`. Two recognized shapes, both requiring a shared non-empty trigger with ≥2 participants: (A) aggregate — ≥2 same-trigger numeric-bounded members with ≥1 carrying a singleton (uncompared) atom; (B) relational — ≥2 members using inter-entity comparison language ("differs from", "the same … as", matched by `hasRelationalLanguage`) — `src/formal/relational.ts:81`, `:116`. It is the opposite of the quantity-alias detector: that one SOLVES the recoverable via propose→commit→z3; this one declines to certify the unrecoverable. Both are demotion-only, both live in `PROPOSE_ONLY_FND_CODES`, neither can ever promote `verified` — `.erpaval/solutions/architecture/detect-and-demote-vs-solve-for-intractable-blind-spots.md`.

### Lint→formal exclusion is loud (`FND_EXCLUDED_FROM_FORMAL`, info, demotion)

A requirement dropped from the solver by an error-severity GtWR/parse finding is no longer silently skipped: `check` emits one `FND_EXCLUDED_FROM_FORMAL` per excluded requirement and demotes `verified` — the fix for the "0 contradictions, verified:true, but a third of the doc was never checked" trap — `src/pipeline/check.ts:1024`, `:1219`. The demotion is driven by the gate's `excluded` set (a structural fact), NOT by the post-waiver finding set — so waiving the disclosure code alone hides the report line but can NEVER promote `verified` over a requirement the solver still never saw — `src/pipeline/check.ts:1191`, `:1215`. Coverage is surfaced first-class as `coverage.{encoded, excluded}` plus a `pairsCheckedNote` so a low pair count is not misread — `src/pipeline/check.ts:1301`. Re-admission is by fixing the lint, or by waiving the *blocking* finding, which the waiver-aware gate honors (below).

---

## Ambiguity family (v3.1)

A deterministic, always-on finding family grounded in the Berry & Kamsties taxonomy: lexical (vague), scope/quantifier, referential/anaphoric, and pragmatic/contextual ambiguity. The module is PURE and SYNCHRONOUS — no solver, no model, no async, no I/O — so it recomputes bit-identical findings and runs on the default `check` path like structural + lint — `src/formal/ambiguity.ts:442`, `src/pipeline/check.ts:691`. Only the mechanical case is verdict-eligible; the rest are `info` / propose-only, tagged `tier: 'lint'` in the report — `src/pipeline/check.ts:695`.
...
The propositional tier evaluates one snapshot; the temporal tier reasons about ORDER over a finite trace. It is opt-in (`--temporal`, default bound `k=10`) and, when enabled, runs over ALL requirements on the shared in-process Z3-WASM context — `src/pipeline/check.ts:871`, `src/cli/index.ts:311`.
...
- [Public API](../reference/public-api.md) — 18 shared source citations
- [Processes](../behavior/processes.md) — 13 shared source citations
- [Contract map](contract-map.md) — 11 shared source citations
- [Impact analysis](impact-analysis.md) — 10 shared source citations
